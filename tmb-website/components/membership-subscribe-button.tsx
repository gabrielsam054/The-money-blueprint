...
export function MembershipSubscribeButton() {
  ...
  async function handleClick() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/membership/subscribe", { method: "POST" });
      ...
      if (!res.ok || !json.authorization_url) {
        throw new Error(json.error ?? "Couldn't start membership signup.");
      }
      window.location.href = json.authorization_url;
    } catch (err) {
      setError(err instanceof Error ? err.message : "Couldn't start membership signup.");
      setLoading(false);
    }
  }
